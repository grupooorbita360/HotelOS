-- Fix real encontrado probando el handoff de demo P0 contra Supabase real:
-- reset_demo_hotel() (0050) fallaba con
--   "update or delete on table \"reservations\" violates foreign key
--    constraint \"leads_reservation_id_fkey\" on table \"leads\""
-- en cuanto el Hotel Demo real tenía, por uso real de la app, algún lead
-- con status='converted' + reservation_id apuntando a una reserva de ese
-- mismo hotel (leads.reservation_id -> reservations.id, 0018). El orden de
-- borrado de 0050 borra `reservations` (hijo->padre, línea 103) ANTES que
-- `leads` (línea 107) -- pero `leads.reservation_id` referencia justo la
-- fila que se intenta borrar primero, sin `on delete` alguno (0018), así
-- que Postgres rechaza el DELETE de reservations mientras exista un lead
-- convertido que la referencie.
--
-- No es un problema del seed en sí (0050 nunca inserta leads, ver la
-- sección "Handoff de demo P0" de CLAUDE.md) -- es un hueco real en el
-- orden de limpieza que sólo se manifiesta cuando el hotel demo real ya
-- acumuló uso genuino (leads convertidos a reserva) entre un reset y el
-- siguiente. Nunca se detectó antes porque las pruebas de 0050 se hicieron
-- contra un hotel demo recién creado, sin leads convertidos todavía.
--
-- Corrección mínima: antes de borrar `reservations`, se desvincula
-- (reservation_id = null) cualquier lead del hotel que apunte a una de sus
-- reservas -- el lead en sí se sigue borrando dos líneas después, como
-- siempre; sólo se rompe la referencia primero para no bloquear el borrado
-- de la reserva. Ningún otro cambio de comportamiento: mismo wipe, mismo
-- seed, misma firma (create or replace, sin tocar parámetros).
...
begin
  ...
  -- Fix de esta migración: desvincula leads convertidos ANTES de borrar
  -- reservations, para no violar leads_reservation_id_fkey (ver comentario
  -- de cabecera). El lead se borra normal dos líneas después.
...
comment on function public.reset_demo_hotel() is
  'Borra y regenera el Hotel Demo (slug hotel-demo) con ~7 semanas de historial realista. Sólo platform_admin. Idempotente y atómico. Desvincula leads convertidos antes de borrar reservations (0053) para no violar leads_reservation_id_fkey.';
