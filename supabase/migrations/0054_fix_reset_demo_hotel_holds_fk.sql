-- Segundo fix real de la misma familia que 0053, encontrado en la MISMA
-- ronda de pruebas contra Supabase real (reset inmediato después de aplicar
-- 0053): reset_demo_hotel() volvió a fallar, ahora con
--   "update or delete on table \"reservations\" violates foreign key
--    constraint \"inventory_holds_converted_reservation_id_fkey\"
--    on table \"inventory_holds\""
--
-- Causa idéntica a 0053, columna distinta: confirm_reservation_from_hold()
-- (0016, línea ~250) fija SIEMPRE `inventory_holds.status = 'converted'` +
...
-- de Reservaciones deja esta referencia. `inventory_holds.
-- converted_reservation_id -> reservations.id` (0018) tampoco tiene
-- `on delete`, y el wipe de 0050 borra `reservations` (línea 92, ya
-- renumerada por 0053) antes que `inventory_holds` (línea 93) -- mismo
-- patrón exacto que el lead convertido de 0053, sólo que aquí el hueco es
-- más grave: no es un caso raro, es EL camino normal de confirmar una
-- reserva, así que el reset fallaría casi con seguridad contra cualquier
-- hotel demo con uso real, no sólo ocasionalmente.
--
-- Corrección: mismo patrón que 0053 -- desvincular converted_reservation_id
-- antes del wipe, en vez de reordenar todo el bloque de DELETEs (que
-- tendría su propio riesgo: inventory_holds también puede depender de
-- quote_options, y reordenar a ciegas podría introducir un problema
-- distinto). Ningún otro cambio de comportamiento.
...
begin
  ...
  -- Fix de esta migración (0054): mismo problema, columna distinta --
  -- desvincula Holds ya convertidos ANTES de borrar reservations, para no
  -- violar inventory_holds_converted_reservation_id_fkey (ver comentario
  -- de cabecera). El Hold se borra normal dos líneas después, sin cambio.
...
comment on function public.reset_demo_hotel() is
  'Borra y regenera el Hotel Demo (slug hotel-demo) con ~7 semanas de historial realista. Sólo platform_admin. Idempotente y atómico. Desvincula leads convertidos (0053) y Holds convertidos (0054) antes de borrar reservations, para no violar sus FK.';
