-- HotelOS — reset_demo_hotel(): agrega cash_movements/cash_shifts al wipe.
--
-- Bug real encontrado en esta sesión: al aplicar 0046_caja_module.sql por
-- primera vez y generar movimientos de Caja reales contra el Hotel Demo
-- (turno abierto, cobros/reembolsos, register_payment_with_movements()),
-- volver a correr reset_demo_hotel() falló con:
...
-- función wipe de reset_demo_hotel() (0050, recreada en 0053/0054/0055) es
-- de antes de que Caja (0046) existiera y nunca se actualizó para
-- contemplar cash_movements/cash_shifts -- ninguna de las dos tablas se
-- borra en ningún punto de la función.
--
-- El pedido original fue "desvincular cash_movements antes de borrar
-- stay_transactions, mismo patrón que 0053/0054 (leads/inventory_holds)".
-- Investigado antes de aplicarlo literal (regla de este proyecto: verificar
-- contra el código real, no asumir): el patrón 0053/0054 funciona porque en
-- ambos casos la FILA que queda con la referencia en null (el lead, el
-- Hold) se BORRA ella misma unas líneas después en la misma función -- un
-- "desvincular" ahí es sólo un paso intermedio antes de un delete real, no
-- un estado final. Aquí no hay un delete de cash_movements/cash_shifts en
-- ningún lado de la función: poner sus FKs en null y nunca borrar la fila
-- dejaría huérfanos permanentes que se acumulan en cada reset futuro --
-- exactamente lo que la sección "Bug real" de este mismo archivo (0053)
-- advierte que hay que evitar.
--
-- Por eso el fix real es BORRAR cash_movements y cash_shifts del hotel
-- demo (scoped por hotel_id, igual que las otras 16 tablas del wipe), no
-- desvincular sus columnas -- es la operación correcta para lo que 0053/
-- 0054 modelan como "limpiar un wipe que faltó actualizar", aplicada al
-- caso real de esta tabla en vez de la desvinculación literal pedida.
-- payment_methods/cash_settings (config del hotel, mismo nivel que
-- hotel_policies/reception_settings) se conservan sin tocar -- no son
-- datos operativos del ciclo de vida que este wipe reinicia.
--
-- Orden: cash_movements primero (su FK a cash_shifts, cash_shift_id,
-- tampoco tiene "on delete cascade"), luego cash_shifts -- antes de
-- stay_transactions, que es lo que cash_movements referencia.
--
-- Migración recreada completa (create or replace, sin cambio de firma) a
-- partir de la versión más reciente (0055) -- mismo criterio ya usado en
-- 0053/0054/0055 para no editar una migración ya aplicada.
...
begin
  ...
  -- ── 1. Hotel demo ────────────────────────────────────────────────────
  -- Fuente de verdad: hotels.is_demo (0055). El slug 'hotel-demo' se
  ...
  -- Fix de 0053: desvincula leads convertidos ANTES de borrar reservations,
  -- para no violar leads_reservation_id_fkey.
  update public.leads set reservation_id = null
  where hotel_id = v_hotel_id and reservation_id is not null;

  -- Fix de 0054: mismo problema, columna distinta -- desvincula Holds ya
  -- convertidos ANTES de borrar reservations, para no violar
  -- inventory_holds_converted_reservation_id_fkey (ver comentario de
  -- cabecera). El Hold se borra normal dos líneas después, sin cambio.
  update public.inventory_holds set converted_reservation_id = null
  where hotel_id = v_hotel_id and converted_reservation_id is not null;

  -- Fix de esta migración (0062): cash_movements.stay_transaction_id /
  -- .payment_movement_id no tienen "on delete cascade" -- a diferencia de
  -- leads/inventory_holds, aquí no hay un "desvincular y borrar después":
  -- se borran las dos tablas de Caja directamente (scoped a este hotel),
  -- antes de tocar stay_transactions. cash_movements primero (referencia a
  -- cash_shifts, tampoco cascada). payment_methods/cash_settings (config
  -- del hotel) no se tocan.
  delete from public.cash_movements where hotel_id = v_hotel_id;
  delete from public.cash_shifts    where hotel_id = v_hotel_id;
  ...
  insert into public.delivered_assets (hotel_id, stay_id, asset_name, delivered, delivered_at, returned, returned_at)
  select v_hotel_id, st.id, 'Control TV', true, checked_in_at, false, null
  from public.stays st
  join public.reservation_stays rs on rs.id = st.reservation_stay_id
  join public.room_types rt on rt.id = rs.room_type_id
...
comment on function public.reset_demo_hotel() is
  'Borra y regenera el Hotel Demo (hotels.is_demo = true) con ~7 semanas de historial realista. Sólo platform_admin. Idempotente y atómico. Incluye el wipe de cash_movements/cash_shifts (0062).';
